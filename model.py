...
class ManageLogin(DataManager):
    def setAkses(self, id_pegawai, tgl_login):
        self.query = 'INSERT INTO akses (id_pegawai, tgl_login) VALUES (\'%s\', \'%s\')'
        self.query = self.query % (id_pegawai, tgl_login)
        if self.isDebug:
    ...
    def getAkses(self):
        self.query = 'SELECT id_pegawai from akses'
        if self.isDebug:
            print('self.query : ', self.query )
        idtransaksi, errMsg = self.executeQuery(self.query, retVal=True)
        return idtransaksi, errMsg

    def getUsernamePass(self):
        self.query = 'SELECT id_pegawai, firstname_pegawai, lastname_pegawai, nohp_pegawai, email_pegawai, username, password from pegawai'
